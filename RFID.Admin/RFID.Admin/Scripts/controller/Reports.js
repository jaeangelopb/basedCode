    ...
    var ReportsController = function ($q, $rootScope, $scope, $location, $mdDateRangePicker, commonRequest) {
        ...
        $scope.ExportAttendanceLogs = function () {
            ...
            if ($scope.attendance.CompanyID !== "-")
                paramstring += "comp=" + $scope.attendance.CompanyID + "&";
             

            if ($scope.dateRange.dateStart)
                paramstring += "sdate=" + moment($scope.dateRange.dateStart).format("YYYY-MM-DD") + "&";
            if ($scope.dateRange.dateEnd)
                paramstring += "edate=" + moment($scope.dateRange.dateEnd).format("YYYY-MM-DD");


            window.open(
              url + "/Reports/AttendanceLogs.aspx?" + paramstring,
              '_blank' 
            );
        };
