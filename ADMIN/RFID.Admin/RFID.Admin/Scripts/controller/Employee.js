(function () {
    ...
    var employeeController = function ($q, $rootScope, $scope, $mdToast, commonRequest) {
        ...
        var List = function () {
            ...
            var onSuccessful = function (response) {
                ...
                if ($scope.checkAll) {
                    var list = $scope.list.data.map(a => a.EmployeeID);
                    $scope.checklistary.push(list);
                }
            };

            var param = $.param({
                "CompanyID": $scope.list.CompanyID,
        ...
        var GetAllAffiliation = function () {
            $scope.Affiliation.loading = true;
            var onSuccessful = function (response) {
                $scope.Affiliation.data.AffiliationList = response.data.AffiliationList;
                $scope.Affiliation.total = response.total;
                $scope.Affiliation.totalPage = Math.ceil($scope.list.Affiliation / $scope.Affiliation.PageSize);
                $scope.Affiliation.loading = false;
                if ($scope.checkAll) {
                    var list = $scope.Affiliation.data.map(a => a.EmployeeID);
                    $scope.checklistary.push(list);
                }
            };

            var param = $.param({
                "CompanyID": $scope.Affiliation.CompanyID,
                "search": $scope.Affiliation.Search,
                "pageindex": $scope.Affiliation.PageIndex,
                "pagesize": $scope.Affiliation.PageSize
            });


            commonRequest.GetData(controller, "GetAllAffiliation", param)
                .then(onSuccessful, onError);
        ...
        var Post = function () {
            ...
            var onSuccessful = function (response) { 
                if (response.MemberID !== "00000000-0000-0000-0000-000000000000") {
                    if ($scope.mode === "add")
            ...
            commonRequest.SubmitForm(controller, "SaveEmployee", formdata)
                .then(onSuccessful, onError);
        ...
        var UploadPost = function () {
            ...
            for (x in emp) {
                ...
                formdata.append("Employee.Affiliation", emp[x].Affiliation);
                formdata.append("Employee.Department", emp[x].Department);
                formdata.append("Employee.SchoolYear", emp[x].SchoolYear);
                formdata.append("Employee.IDNumber", emp[x].IDNumber);
                formdata.append("Employee.MobileNumber", emp[x].MobileNumber);
                formdata.append("Employee.RFID", emp[x].RFID);
                formdata.append("Employee.ProfilePhoto", emp[x].ProfilePhoto);
        ...
        //        if (!hasAll) {
        //            $scope.list.CompanyID = $scope.companylist[0].CompanyID;
        //        }
        //    }
        ...
        $scope.UploadCSV = function (file) {
            ...
            reader.onload = (function (file) {
                return function (evt) {
                    ...
                    try {
                        var fields = ["FirstName", "MiddleName", "LastName", "EmailAddress", "Affiliation", "Department", "SchoolYear", "IDNumber","MobileNumber", "RFID", "ProfilePhoto"];
        ...
        $scope.ToView = function (data) {
            ...
            GetAllAffiliation();
            $scope.Affiliation.data.AffiliationList = [];
        }
        ...
        $scope.downloadTemplate = function () {
            ...
            var data = 'FirstName,MiddleName,LastName,EmailAddress,Affiliation,Department,SchoolYear,IDNumber,MobileNumber,RFID,ProfilePhoto';
        ...
        $scope.GetAllAffiliation = GetAllAffiliation;
        $scope.List = List;
        $scope.Init = Init;
    ...
    angular.module("mainApp")
        .controller("EmployeeController", ['$q', '$rootScope', '$scope', '$mdToast', 'commonRequest', employeeController]);
